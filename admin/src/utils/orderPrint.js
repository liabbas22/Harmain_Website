...
export const printKitchenReceipt = (order) => {
  ...
  const itemRows = items.map((item) => {
    ...
    return `<li><span class="quantity">x${totalQuantity}</span><span class="item-name">${escapeHtml(item.name || "Menu item")}</span>${item.optionName ? `<span class="option">${escapeHtml(item.optionName)}</span>` : ""}${comboItems}${addOns}${freeQuantity > 0 ? `<div class="kitchen-note">Includes ${freeQuantity} free item${freeQuantity > 1 ? "s" : ""} from offer</div>` : ""}${item.specialInstructions ? `<div class="kitchen-note">Special instruction: ${escapeHtml(item.specialInstructions)}</div>` : ""}</li>`;
  }).join("");
