...
const normalizeStatusKey = (value, fallback = "not_required") => {
...
const mergeRefundStatuses = (rows) =>
  Array.from(
    safeList(rows)
      .reduce((map, entry) => {
        const status = normalizeStatusKey(entry.status);
        const current = map.get(status) || { status, orders: 0, amount: 0 };
...
function CancellationReport({ data }) {
  ...
  const refundStatuses = mergeRefundStatuses(data?.byRefundStatus);
        ...
        {[0, 1, 2].map((item) => (
          ...
          >
            ...
            <div className="grid content-start gap-4 p-5">
              {[0, 1, 2, 3, 4].map((row) => (
                <div key={row} className="grid grid-cols-[92px_minmax(0,1fr)_80px] items-center gap-3">
