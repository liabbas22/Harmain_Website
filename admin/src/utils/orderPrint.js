...
export const printOrderInvoice = (order) => {
  ...
  const discountLabel = order.coupon?.code ? `Coupon (${escapeHtml(order.coupon.code)})` : order.offer?.name ? `Offer (${escapeHtml(order.offer.name)})` : "Discount";
  const offerBreakdown = (order.offerBreakdown || []).map((detail) => `<div class="total-line muted"><span>${Number(detail.quantity || 0)} x ${escapeHtml(detail.productName || "Menu item")} - ${escapeHtml(detail.offerName || "Offer")}</span><strong>- ${escapeHtml(money(detail.discount))}</strong></div>`).join("");
  const discountLine = Number(order.discount) > 0 ? `<div class="total-line"><span>${discountLabel}</span><strong>- ${escapeHtml(money(order.discount))}</strong></div>${offerBreakdown}` : "";
