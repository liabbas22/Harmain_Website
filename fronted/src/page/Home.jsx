...
const productCard = (product) => ({ id: product._id, title: product.name, description: product.description, image: product.image || AssetsData.ChickenCheeseOmellete, options: product.options?.length ? product.options : [{ name: "Regular", actualPrice: product.price }] });
...
const Home = () => {
  ...
  return <div className="bg-red-50"><Herosection />{loading ? <div className="mx-4 py-8 md:mx-8 lg:mx-24"><Skeleton /></div> : error ? <div className="mx-4 py-8 md:mx-8 lg:mx-24"><State error={error} retry={loadMenu} /></div> : !categoryUi.length ? <div className="mx-4 py-8 md:mx-8 lg:mx-24"><State /></div> : <><Category categories={categoryUi} /><div className="flex flex-col gap-4 py-2 mx-4 my-auto md:mx-8 lg:mx-24"><Search /><PopularItem items={popular} /><CategoryData categories={categoryUi} /></div></>}</div>;
};
export default Home;
