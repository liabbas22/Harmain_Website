    ...
    <>
      ...
      >
        <div className="relative w-full h-48 overflow-hidden md:h-52 lg:h-60 rounded-xl">
          ...
          {!isAvailable && (
            <span
              className={`absolute ${item?.activeOffer ? "top-12" : "top-3"} right-3 z-10 rounded-md bg-red-700 px-2 py-1 text-[10px] font-extrabold uppercase tracking-wide text-white shadow-sm`}
            >
              {availabilityLabel}
            </span>
          )}
          <h3
            className={`absolute left-3 text-sm font-bold tracking-wide text-white md:text-base lg:text-lg line-clamp-2 ${item?.activeOffer ? "right-28 top-4" : "right-3 top-3"}`}
          >
            {item?.title}
          </h3>
