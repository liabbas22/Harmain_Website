...
export const printOrderInvoice = (order) => {
  ...
  const discountLine = Number(order.discount) > 0 ? `<div class="total-line"><span>Total savings</span><strong>- ${escapeHtml(money(order.discount))}</strong></div>${couponLine}${offerLine}${offerBreakdown}` : "";
...
export const printKitchenReceipt = (order) => {
  ...
  const itemRows = items.map((item) => {
    ...
    const freeQuantity = Number(item.freeQuantity || 0);
    const totalQuantity = paidQuantity + freeQuantity;
    return `<li><span class="quantity">x${totalQuantity}</span><span class="item-name">${escapeHtml(item.name || "Menu item")}</span>${item.optionName ? `<span class="option">${escapeHtml(item.optionName)}</span>` : ""}${freeQuantity > 0 ? `<div class="kitchen-note">Includes ${freeQuantity} free item${freeQuantity > 1 ? "s" : ""} from offer</div>` : ""}${item.specialInstructions ? `<div class="kitchen-note">Special instruction: ${escapeHtml(item.specialInstructions)}</div>` : ""}</li>`;
  }).join("");
